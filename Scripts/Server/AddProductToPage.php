<?php
    ...
    $prepareProductStatement->bind_param('ss', $_POST["product_type"], $_POST["product_colour"]);
    $prepareProductStatement->execute();
    $result = $prepareProductStatement->get_result();
    $data_row = $result->fetch_assoc();
    echo '<div class="row" id="' . $_POST["product_type"] . $_POST["product_colour"] . '">';
    ...
    echo '<button onclick="removeFromCart(\'' . $_POST["product_type"] . $_POST["product_colour"] . '\', \'' . $_POST["localStorageKey"] . '\')">Remove</button>';
    echo '</div>';
?>